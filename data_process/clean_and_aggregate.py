...
############      python clean_and_aggregate.py --state 6 --county 37 --year 2015 2025        ############
############                                                                                  ############
##########################################################################################################
...
def detect_counties(years, categories, state_code=6, county_code=37):
    ...
    for cat_key in categories:
        ...
        if cat_results:
            ...
            for pname, df in cat_results.items():
                df.to_csv(os.path.join(out_dir, f"{pname}_daily.csv"), index=False)
            summary[cat_key] = f"{len(cat_results)} files saved"
    return summary
...
def main():
    ...
    print(f"Targeting State: {state_code}, County: {county_code}")
    ...
    for cat_key in categories:
        ...
        for pc, pcfg in cat.get("params", {}).items():
            if pcfg.get("is_aqi"):
                ...
                r, cn = process_one_param(cat["raw_subdir"], pc, pcfg, state_code, county_code, years)
            
            if r is not None:
                cat_results[pcfg["out_name"]] = r
                if cn: county_name = cn

        if cat_results:
            dir_name = f"{state_code:02d}_{county_code:03d}_{county_name}"
            out_dir = os.path.join(".", "data", "processed", dir_name, cat["out_subdir"])
            os.makedirs(out_dir, exist_ok=True)
            for pname, df in cat_results.items():
                save_path = os.path.join(out_dir, f"{pname}_daily.csv")
                df.to_csv(save_path, index=False)
                print(f"  Successfully saved: {save_path}")
            print(f"Finished Category: {cat_key}")
